...
class RoomsRepository(BaseRepository):
    ...
    async def get_filtered_by_time(
        ...
        rooms_count = (
            ...
            .filter(    # в filter_by() нельзя делать сравнения, а в filter() можно
                BookingsOrm.date_from <= date_to,
                BookingsOrm.date_to >= date_from,
            )
            .group_by(BookingsOrm.room_id)
            .cte(name="rooms_count")    # это WITH rooms_count AS
        )
        ...
        rooms_left_table = (
            ...
            .cte(name="rooms_left_table")    # тоже относится к WITH
        )
